  ...
  */
  ...
  function patchProps(element, oldProps, newProps) {
    ...
    for (var _key2 in newProps) {
      if (_key2 === "style") {
        for (var _key3 in newStyle) {
          element.style[_key3] = newStyle[_key3];
        }
      } else {
        element.setAttribute(_key2, newProps[_key2]);
      }
    }
  }
  ...
  function unMountChildren(el, oldChildren) {
    var _iterator2 = _createForOfIteratorHelper(oldChildren),
      _step2;
    try {
      for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
        var childVNode = _step2.value;
